...
TAKE_PROFIT_PCT = 0.10   # 指値(利確)の割合
STOP_LOSS_PCT = -0.03    # 逆指値(損切り)の割合
...
# "-latest"エイリアスは無料枠が厳しいプレビュー版を指すことがあるため、
# 無料枠が手厚い安定版(GA)モデルに固定する。
# エラーが出る場合はGoogle AI Studioのレート制限画面で現在のモデル名・上限を確認してください。
GEMINI_MODEL = "gemini-3.5-flash"
GEMINI_API_URL = f"https://generativelanguage.googleapis.com/v1beta/models/{GEMINI_MODEL}:generateContent"
NEWS_MAX_ITEMS = 5  # 要約に使うニュース見出しの最大件数
...
def fetch_all_latest_data(tickers: list) -> dict:
    ...
    """
    BATCH_SIZE = 200  # 一度に大量の銘柄をダウンロードすると失敗しやすいため分割する
    print(f"{len(tickers)}銘柄のデータを{BATCH_SIZE}銘柄ずつ取得中...")
    ...
    for i in range(0, len(tickers), BATCH_SIZE):
        ...
        try:
            ...
            print(f"  [警告] バッチ{i // BATCH_SIZE + 1}の取得に失敗: {e}")
            continue
        ...
        print(f"  バッチ{i // BATCH_SIZE + 1}完了(累計 {len(data)}銘柄取得済み)")

    print(f"  -> 最終的に{len(data)}銘柄のデータ取得に成功")
    if not data:
        raise RuntimeError("株価データが1件も取得できませんでした。Yahoo側のレート制限や通信エラーの可能性があります。")
    return data
...
def build_decline_ranking(data: dict, top_n: int) -> pd.DataFrame:
    ...
    if ranking.empty:
        raise RuntimeError(
            "値下がりイベントの候補が1件も作れませんでした。"
            "株価データが取得できていない(dataが空/ほぼ空)か、特徴量計算に十分な日数の"
            f"データが無い可能性があります(取得できた銘柄数: {len(data)})。"
        )
...
# =====================================================================
# 4.5 TDnet(適時開示情報)を取得し、Geminiで値下がり要因を要約する
# =====================================================================

TDNET_API_URL = "https://webapi.yanoshin.jp/webapi/tdnet/list/{code}.json"
TDNET_MAX_ITEMS = 5  # 要約に使う開示タイトルの最大件数


def fetch_tdnet_titles(ticker: str, max_items: int = TDNET_MAX_ITEMS) -> list:
    """
    TDnet(適時開示情報)の非公式無料API(やのしんさん提供)から、
    直近の開示タイトルを取得する。
    """
    code = ticker.replace(".T", "")
    url = TDNET_API_URL.format(code=code)

    try:
        resp = requests.get(url, params={"limit": max_items}, timeout=15)
        resp.raise_for_status()
        data = resp.json()
    except Exception as e:
        print(f"  [警告] {ticker} のTDnet取得に失敗: {e}")
        return []

    titles = []
    for item in (data.get("items") or [])[:max_items]:
        entry = item.get("Tdnet", item)  # "Tdnet"キーで包まれている場合とそうでない場合の両方に対応
...
def summarize_decline_reason(ticker: str, company_name: str, max_attempts: int = 4) -> str:
    """TDnetの開示タイトルをもとに、Gemini APIで値下がり要因を短く要約する。
    429(レート制限)・503(サーバー混雑)・接続エラーは一時的なことが多いため、
    待機時間を伸ばしながらリトライする。"""
    titles = fetch_tdnet_titles(ticker)
    if not titles:
        return "TDnetに直近の開示情報なし"

    if not GEMINI_API_KEY:
        return "(GEMINI_API_KEY未設定のため要約なし)"

    prompt = (
        f"以下は{company_name}({ticker})に関する直近の適時開示情報(TDnet)のタイトルです。"
        f"株価が値下がりした要因として考えられる点を、40字程度の日本語で簡潔にまとめてください。"
        f"直接的な関連が読み取れない場合は、その旨がわかるように書いてください。"
        f"断定は避け、あくまで参考情報として提示する書き方にしてください。\n\n"
    ...
    for attempt in range(max_attempts):
        try:
            ...
            if attempt < max_attempts - 1:
                print(f"  [情報] {ticker} 要約失敗(試行{attempt + 1}/{max_attempts}): {e}"
                      f" → {wait_seconds}秒待って再試行")
                time.sleep(wait_seconds)
                wait_seconds *= 2

    print(f"  [警告] {ticker} の要約生成に失敗(最終): {last_error}")
    return "(要約生成に失敗しました)"


def build_decline_reasons(ranking: pd.DataFrame) -> dict:
    """ranking内の各銘柄について、値下がり要因の要約を辞書で返す"""
    reasons = {}
    for _, row in ranking.iterrows():
        ticker = row["ticker"]
        company_name = row.get("銘柄名", ticker)
        reasons[ticker] = summarize_decline_reason(ticker, company_name)
        time.sleep(5)  # レート制限(1分15回)に余裕を持たせるための間隔
    return reasons
...
def format_output_table(ranking: pd.DataFrame, decline_reasons: dict = None, computed_at: str = None) -> pd.DataFrame:
    table = ranking.copy()
    ...
    time_suffix = f"({computed_at}時点)" if computed_at else ""
    table["反転確率"] = (table["reversal_probability"] * 100).round(1).astype(str) + "%" + time_suffix
    table["指値"] = (table["close"] * (1 + TAKE_PROFIT_PCT)).round(0)
    table["逆指値"] = (table["close"] * (1 + STOP_LOSS_PCT)).round(0)
    if decline_reasons:
        table["値下がり要因"] = table["ticker"].map(decline_reasons).fillna("-")
    ...
    columns = ["銘柄コード", "企業名", "業種", "時価総額(億円)", "値下がり率", "現在株価", "反転確率", "指値", "逆指値"]
    if decline_reasons:
        columns.append("値下がり要因")
...
def build_flex_carousel(ranking_chunk: pd.DataFrame, image_paths: dict, decline_reasons: dict = None,
                         computed_at: str = None, alt_text: str = "本日の値下がりランキング") -> dict:
    """
    渡されたranking_chunk(最大10件程度を想定)を、1銘柄=1バブル(カード)として
    カルーセルFlexメッセージにする。
    画像はGitHubにpush済みである前提で、raw.githubusercontent.comのURLを組み立てる。
    computed_atを渡すと、反転確率が何時時点の株価に基づく算出かを併記する。
    """
    ...
    for _, row in ranking_chunk.iterrows():
        ...
        time_suffix = f"({computed_at}時点)" if computed_at else ""
        proba_text = f"{proba * 100:.1f}%{time_suffix}" if pd.notna(proba) else "算出不可"
        reason_text = decline_reasons.get(ticker, "-")
                ...
                "contents": [
                    ...
                    },
                    {
                        ...
                        "contents": [
                            ...
                            _flex_row("指値", f"{take_profit_price:.0f}円(+{TAKE_PROFIT_PCT * 100:.0f}%)"),
                            _flex_row("逆指値", f"{stop_loss_price:.0f}円({STOP_LOSS_PCT * 100:.0f}%)"),
                            _flex_row("値下がり要因", reason_text),
                        ],
                    },
                ],
            },
...
if __name__ == "__main__":
    computed_at = datetime.now(ZoneInfo("Asia/Tokyo")).strftime("%H:%M")
    ...
    print("\n値下がり要因の要約を生成中(TDnet取得+Gemini要約)...")
    decline_reasons = build_decline_reasons(ranking)

    table = format_output_table(ranking, decline_reasons=decline_reasons, computed_at=computed_at)
    print(f"\n=== 値下がりランキング TOP {TOP_N}(反転確率つき、{computed_at}時点) ===")
    ...
    for i in range(n_chunks):
        ...
        flex_message = build_flex_carousel(chunk, image_paths, decline_reasons=decline_reasons,
                                            computed_at=computed_at, alt_text=alt_text)
