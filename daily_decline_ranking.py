...
def attach_company_info(ranking: pd.DataFrame, jpx_info: pd.DataFrame) -> pd.DataFrame:
    ...
    market_caps = []
    for ticker, price in zip(ranking["ticker"], ranking["close"]):
        cap = None
        info = {}
        for attempt in range(2):  # 失敗時に1回だけリトライ
            try:
                ...
                if attempt == 0:
                    time.sleep(1)
                    continue
                print(f"  [警告] {ticker} の企業情報取得に失敗: {e}")

        cap = info.get("marketCap")
        if cap is None:
            # marketCapキー自体が無い銘柄向けのフォールバック: 発行済株式数×現在株価
            shares = info.get("sharesOutstanding")
            if shares:
                cap = shares * price
                print(f"  [情報] {ticker} はmarketCap欠落のため sharesOutstanding×株価 で代用計算しました")
            else:
                print(f"  [警告] {ticker} の時価総額を算出できませんでした(marketCap/sharesOutstandingとも無し)")

        market_caps.append(cap)
        time.sleep(0.3)  # 連続アクセスによるレート制限を避ける
...
def format_output_table(ranking: pd.DataFrame) -> pd.DataFrame:
    ...
    return table[["銘柄コード", "企業名", "業種", "時価総額(億円)", "値下がり率", "現在株価", "反転確率", "指値", "逆指値"]]
