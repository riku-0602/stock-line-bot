...
# 無料枠で使える軽量モデル。Googleのモデル改廃が頻繁なため、エラーが出る場合は
# Google AI Studio(https://aistudio.google.com/)で現在の無料対象モデル名を確認して変更してください。
...
# =====================================================================
# 4.5 ニュース見出しを取得し、Geminiで値下がり要因を要約する
# =====================================================================

def fetch_news_titles(ticker: str, max_items: int = NEWS_MAX_ITEMS) -> list:
    """yfinanceのニュース見出しを取得する(構造がバージョンにより異なるため複数キーを試す)"""
    try:
...
def summarize_decline_reason(ticker: str, company_name: str) -> str:
    """ニュース見出しをもとに、Gemini APIで値下がり要因を短く要約する"""
    ...
    prompt = (
        f"以下は{company_name}({ticker})に関する直近のニュース見出しです。"
...
def format_output_table(ranking: pd.DataFrame, decline_reasons: dict = None) -> pd.DataFrame:
    table = ranking.copy()
...
def build_flex_carousel(ranking_chunk: pd.DataFrame, image_paths: dict, decline_reasons: dict = None,
                         alt_text: str = "本日の値下がりランキング") -> dict:
    """
...
if __name__ == "__main__":
    ...
    print("\n値下がり要因の要約を生成中(ニュース取得+Gemini要約)...")
    decline_reasons = build_decline_reasons(ranking)

    table = format_output_table(ranking, decline_reasons=decline_reasons)
    ...
    for i in range(n_chunks):
        ...
        flex_message = build_flex_carousel(chunk, image_paths, decline_reasons=decline_reasons, alt_text=alt_text)
